...
Acts as the persistent database. The GitHub Action (and local pipeline) references this repo and runs the pipeline **directly against the data repository's folder**. No fetched or extracted data is ever copied into or written to the master code repository. This ensures strict separation between code logic and data state, and maintains data persistence across runs.
