...
def evaluate(record, settings):
    ...
    market_cap_floor = record.static.market_cap * 0.0001
    ...
    is_low_turnover = avg_turnover < dynamic_turnover_threshold

    # Range Ratio Extreme Check
    closes = [h.close for h in H if h.close > 0]
    hist_high = max(closes) if closes else 0
    hist_low = min(closes) if closes else 0
    current_close = H[-1].close
    range_ratio = (hist_high - hist_low) / current_close if current_close > 0 else 0.0
    range_ratio_ok = range_ratio <= settings.range_ratio_max_extreme
    ...
    # Price trend bonus
    price_trend_bonus = 2.0 if price_trend_ok else 0.0
    
    score = round(turnover_pts) + round(appearances_pts) + round(accumulation_bonus) + round(coiling_bonus) + round(price_trend_bonus)

    return hard_pass, is_low_turnover, beta_in_range, score